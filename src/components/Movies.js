...
function Movies(props) {
  ...
  const fetchData = async ()=>{
       let res = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}`);
      
       setMovie(res.data.results);
  }
 
  const nextData = async ()=>{
    let res = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&page=${page + 1}`);
    setPage(page + 1);
    setMovie(res.data.results);
